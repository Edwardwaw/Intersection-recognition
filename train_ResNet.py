...
def train(epoch,trainloader,net,loss_function,optimizer):
    ...
    for batch_index, (images, labels)  in enumerate(trainloader):
        ...
        if batch_index % 10 == 0:
            print('iter = ', batch_index, 'loss = ', loss.item(), 'acc1 = ', acc1/images.shape[0], 'acc5 = ', acc5/images.shape[0])
...
if  __name__ == "__main__":
    ...
    parser.add_argument('--valsize', type=float, default=VAL_SIZE, help='ratio of the size of test(val) set and train set')
    ...
    trainloader, testloader = get_train_test_dataloader(TRAIN_MEAN, TRAIN_STD, batch_size=args.batch_size, num_workers=args.workers, valsize=args.valsize)
    ...
    checkpoint_path = os.path.join(CHECKPOINT_PATH, "ResNet50", TIME_NOW)
    ...
    for epoch in range(args.epochs):
        ...
        if best_acc < acc:
            torch.save(net.state_dict(), checkpoint_path.format(net='Resnet50', epoch=epoch, type='best'))
        torch.save(net.state_dict(), checkpoint_path.format(net='Resnet50', epoch=epoch, type='regular'))
